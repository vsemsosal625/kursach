...
// ГРУППИРУЕМ ПО КАТЕГОРИЯМ (извлекаем из content)
...
// Если выбран фильтр, оставляем только нужную категорию
...
<body>
    <nav class="top-navbar">
        ...
        <div class="settings-icon" onclick="window.location.href='settings.php'"><i class="fas fa-cog"></i></div>
    </nav>
        ...
        <div class="page-header">
            ...
            <div class="controls">
                <div class="filter-group">
                    ...
                    <div class="filter-dropdown">
                        <button class="filter-btn" onclick="toggleFilter()">
                            ...
                            $filterEmojis = ['Легкая линия' => '⚔️', 'Сложная линия' => '🛡️'];
                            echo ($filterEmojis[$categoryFilter] ?? '') . ' ' . $categoryFilter;
                            } else {
                            echo '🌐 Все линии';  } ?>
                            </span>
                        ...
                        <div class="filter-options" id="filterOptions">
                            <div class="filter-option <?= empty($categoryFilter) ? 'active' : '' ?>" onclick="applyFilter('')">🌐 Все линии</div>
                            <div class="filter-option <?= $categoryFilter === 'Легкая линия' ? 'active' : '' ?>" onclick="applyFilter('Легкая линия')">⚔️ Легкая линия</div>
                            <div class="filter-option <?= $categoryFilter === 'Сложная линия' ? 'active' : '' ?>" onclick="applyFilter('Сложная линия')">🛡️ Сложная линия</div>
                        </div>
                    </div>
                </div>
                <div class="filter-group">
                ...
                <div class="search-box">
                    ...
                    <input type="text" class="search-input" id="searchInput" placeholder="Поиск связок..." value="<?= htmlspecialchars($searchQuery) ?>">
                </div>
            </div>
        </div>
        
            <?php 
            $totalSynergies = array_sum(array_map('count', $synergies));
            ?>
            <?php if ($totalSynergies === 0): ?>
                <div style="text-align: center; padding: 60px 20px; color: #8f98a0;">
                    <i class="fas fa-search" style="font-size: 64px; margin-bottom: 20px; color: #4a5568;"></i>
            ...
            <?php foreach ($synergies as $laneType => $laneSynergies): ?>
                ...
                <div class="category-section">
                    <h2 class="category-title">
                        <?= $laneType === 'Легкая линия' ? '⚔️' : '🛡️' ?> <?= htmlspecialchars($laneType) ?>
                    </h2>
                    <div class="synergy-grid">
                        <?php foreach ($laneSynergies as $syn): ?>
                            ...
                            foreach ($heroMatches as $index => $match): ?>           
                                <div class="hero-item">
                                <div class="hero-name"><?= htmlspecialchars($match[1]) ?></div>
                                <div class="hero-pos"><?= $match[2] ?> позиция</div>
                                </div>
                             <?php if ($index < $heroCount - 1): ?>
                                <div style="display: flex; align-items: center; color: #8b5cf6; font-size: 20px; font-weight: 700; padding: 0 10px;">+</div>
                             <?php endif; ?>
                            <?php endforeach; ?>
                                </div>
    ...
    function applyFilter(cat) { 
        ...
        if(cat) {
            ...
            document.getElementById('currentFilterText').textContent = '🌐 Все линии';
        }
