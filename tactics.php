...
switch ($sortBy) {
    ...
    case 'category': $sql .= " ORDER BY category ASC, title ASC"; break;
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-option.active, .sort-option.active { background: rgba(245,158,11,0.3); color: #fbbf24; font-weight: 600; }
        ...
        .search-input:focus { outline: none; border-color: #f59e0b; width: 240px; }
        ...
        .category-heading { color: #fff; font-size: 24px; font-weight: 700; margin-bottom: 20px; padding-left: 15px; border-left: 4px solid #f59e0b; display: flex; align-items: center; gap: 10px; }
        .mechanics-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 25px; }
        .mechanic-card { background: linear-gradient(135deg, #1b2838 0%, #2a475e 100%); border: 1px solid #36414d; border-radius: 12px; padding: 25px; transition: all 0.3s; cursor: pointer; text-decoration: none; color: inherit; display: flex; flex-direction: column; }
        .mechanic-card:hover { transform: translateY(-5px); border-color: #f59e0b; box-shadow: 0 8px 25px rgba(245,158,11,0.3); }
        .mechanic-card .icon-wrap { width: 50px; height: 50px; background: rgba(245,158,11,0.15); border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-bottom: 15px; color: #fbbf24; font-size: 24px; }
        .mechanic-card h3 { color: #fff; font-size: 20px; margin-bottom: 8px; font-weight: 600; }
        .mechanic-card .category { display: inline-block; background: rgba(245,158,11,0.2); color: #fcd34d; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; margin-bottom: 12px; width: fit-content; }
        .mechanic-card .preview { color: #acb2b8; font-size: 14px; line-height: 1.6; flex: 1; }
...
<body>
    ...
    <div class="main-wrapper">
        <div class="page-header">
            <h1 class="page-title">📋 Функциональные роли игроков</h1>
            <div class="controls">
                <div class="filter-group">
                    ...
                    <div class="filter-dropdown">
                        ...
                        <div class="filter-options" id="filterOptions">
                            <div class="filter-option <?= empty($categoryFilter) ? 'active' : '' ?>" onclick="applyFilter('')">
                                🌐 Все позиции
                            </div>
                ...
                <div class="search-box">
                    ...
                    <input type="text" class="search-input" id="searchInput" placeholder="Поиск..." value="<?= htmlspecialchars($searchQuery) ?>">
                </div>
            </div>
        </div>
                    ...
                    <div class="mechanics-category-block">
                        ...
                        <div class="mechanics-grid">
                            <?php foreach ($groupedMechanics[$cat] as $m): ?>
                                <a href="mechanic.php?id=<?= $m['id_game_mechanic'] ?>" class="mechanic-card">
                                    <div class="icon-wrap">
                                        <i class="fas fa-user"></i>
                                    </div>
                                    <span class="category"><?= htmlspecialchars($m['category']) ?></span>
                                    <h3><?= htmlspecialchars($m['title']) ?></h3>
                                    <div class="preview"><?= htmlspecialchars(mb_substr($m['content'], 0, 200)) ?>...</div>
                                </a>
                            <?php endforeach; ?>
                        </div>
                    </div>
    ...
    <script>
        ...
        function applyFilter(cat) { 
            ...
            if(cat) {
                ...
                const emojis = {'Основа (1-3 позиция)': '🛡️', 'Поддержка (4-5 позиция)': '💚'};
                document.getElementById('currentFilterText').textContent = (emojis[cat] || '') + ' ' + cat;
            } else {
                url.searchParams.delete('category'); 
                document.getElementById('currentFilterText').textContent = '🌐 Все позиции';
            }
